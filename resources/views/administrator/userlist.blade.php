                              ...
                              ViewDocument
							       ...
							       <td>{{date('d-m-Y',strtotime($user->created_at))}}</td>
							        <td>
							        	...
							        	@if($user->status=='0')
							        	<a class="btn btn-danger btn-sm" href="{{url('administrator/blockuser/'.$user->id)}}" title="Sbloccare" onclick="return confirm('Sei sicuro di voler sbloccare questo utente')"><i class="fa fa-ban" aria-hidden="true"></i></a>
							        	@else
							        	<a class="btn btn-success btn-sm" href="{{url('administrator/blockuser/'.$user->id)}}" title="Bloccare" onclick="return confirm('Sei sicuro di voler bloccare questo utente')"><i class="fa fa-ban" aria-hidden="true"></i></a>
							        	@endif
                                               
							        </td>
                      <td>
                        <label class="switch"><input type="checkbox" id="togBtn" data-id="{{$user->id}}" name="founder" value="" {{($user->founder=='1')?'checked':''}}><div class="slider round"></div></label>

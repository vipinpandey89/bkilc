                     ...
                     <div class="col-md-4 text-right">
                        <div class="breadcumb">
                            <ul>
                                ...
                                <li><a class="current" href="{{url('user-wallet')}}">E-wallet</a></li>
                                ...
                                <table class="table user-data-table table-striped table-bordered table-hover table-green" id="dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td>{{$detail->name}}</td>
                                             <td>{{$detail->email}}</td>
                                            <td class="center">&euro;{{$detail->commission_point}}</td>
                                           <td>{{date('d-m-Y',strtotime($detail->commissiondate))}}</td>
				...
				<form method="post">  
                            ...
                            <div class="profile-inner">
                              <!---<h3>Richiedi pagamento.</h3>-->
                               <span><strong style="color: #00ade9;">Conversione di {{!empty($getDetail)?$getDetail->convertion_pv:''}} PV = 1 Euro </strong></span>
                              <div class="lavel-upgrade">
                                <ul>
                                  <li class="input-detail"><span>Inserisci l'importo</span> 
                                
                                   <input type="text" name="" value="{{ (count($userProfile)>0)?$userProfile->total_wallet:'0'}}" disabled="" >    <span>&euro;</span>
                               </li>
                                  ...
                                  <li class="money-input">    <input type="text" name="wallet_pv" value="" required> <span>P.V.</span>        </li>
                                   ...
                                   <input type="hidden" name="pv_shouldbe" value="{{!empty($getDetail)?$getDetail->convertion_pv:''}}" > 

                                    <input type="hidden" name="level_euro" value="{{!empty($getDetail)?$getDetail->convertion_euro:''}}" > 


                                   <input type="hidden" name="total_walletpv" value="{{ (count($userProfile)>0)?$userProfile->total_wallet:'0'}}" > 
                                </ul>
                              </div>
                              ...
                              ?>
                                <div class="alert alert-danger">
                                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                                      <strong>Per favore Compelete {{!empty($getDetail)?$getDetail->convertion_pv:''}}.PV per convertire Pv in Euro.</strong> 
                                </div>
                              <?php 
                              }?>
                            </div>  

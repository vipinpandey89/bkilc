      ...
      <div class="row">
            ...
            @foreach($getnews as $newsDetail)
                  <div class="post-item isotope-item clearfix">
                    <div class="date_label">{{date('d-m-Y',strtotime($newsDetail->start_time))}} To {{date('d-m-Y',strtotime($newsDetail->end_time))}} </div>

                    @if(!empty($newsDetail->image))
                    <div class="image_frame post-photo-wrapper image">
                      <div class="image_wrapper">
                        <a href="{{url('meeting-detail/'.$newsDetail->id)}}">
                    ...
                    <div class="post-desc-wrapper">
                      <div class="post-desc">
                        <div class="post-title">
                          <h2 class="entry-title"><a href="{{url('meeting-detail/'.$newsDetail->id)}}">{{$newsDetail->title}}</a></h2>
                        </div>
                        <div class="post-excerpt"><p class="big">{{strip_tags(substr($newsDetail->description,0,200))}}</p></div>
                        <div class="post-footer">
                          <div class="post-links"><i class="fa fa-file-text-o"></i> <a href="{{url('meeting-detail/'.$newsDetail->id)}}" class="post-more">Leggi di più</a></div></div>
                        </div>
                      </div>
